...
```
camera_sdk_python/
├── camera_client.py        # Python 包装层，统一导出 CameraClient
├── CMakeLists.txt          # Python 3.12 native 扩展构建配置
├── lib/
│   ├── __init__.py
│   └── _camera_client_native.cpython-312-x86_64-linux-gnu.so # 编译生成的 native 扩展
├── requirements.txt        # 项目依赖清单
├── proto/                  # gRPC 协议定义目录
│   ├── robot_pb2.py       # 基础数据协议定义（自动生成）
│   └── robot_pb2_grpc.py  # gRPC 服务接口定义（自动生成）
├── src/
│   └── camera_bindings.cpp # pybind11 绑定源码
├── tools/
│   └── build_py312.sh      # Python 3.12 native 扩展构建脚本
├── example/                # 示例脚本目录
...
### 4.0 Python 3.12 编译版
原始交付包里的 Python 3.10 预编译扩展只能被 Python 3.10 导入；当前目录已改为通过 `camera_client.py` 包装 Python 3.12 native 扩展。Python 3.12 环境请先编译本目录新增的 native 扩展：

```bash
cd camera_sdk_python
PYTHON_BIN=/home/mrgh/miniconda3/envs/zerith_h1_312/bin/python tools/build_py312.sh
```

编译后会生成：

```text
lib/_camera_client_native.cpython-312-x86_64-linux-gnu.so
```
...
注意：`camera_sdk_cpp/3rd_party/ffmpeg_static_lib` 里的 FFmpeg 静态库不是 `-fPIC` 编译，不能直接打进 Python `.so`。构建脚本会使用目标 Python/Conda 环境里的共享 FFmpeg 库；如果环境里没有，请先安装：

```bash
conda install -n zerith_h1_312 -c conda-forge ffmpeg
```
